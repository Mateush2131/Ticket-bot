...
def get_role_value(role):
    """Безопасное получение значения роли"""
...
async def select_user(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(info_text, parse_mode=None, reply_markup=keyboard)
...
async def back_to_users(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(stats_text, parse_mode=None)
    ...
    try:
        ...
        for w in pending_withdrawals:
            ...
            if user:
                ...
                await message.answer(withdraw_text, parse_mode=None, reply_markup=get_withdraw_keyboard(w.id))
...
async def back_to_main_admin(message: Message):
    if message.from_user.id in ADMIN_IDS:
        ...
        try:
            ...
            if user:
                role_value = get_role_value(user.role)
                await message.answer("✅ Главное меню", parse_mode=None, reply_markup=get_main_keyboard(role_value))
            else:
